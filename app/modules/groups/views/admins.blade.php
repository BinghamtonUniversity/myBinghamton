<script>
		...
		$.ajax({
			url:'/group_admins?group='+id,
			success: function(data){
				$('#content').html('<div class="panel-page"><section class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><span class="fa fa-lock"></span>&nbsp;Group Admins<span class="btn btn-success pull-right"><i class="fa fa-plus"></i> Add Admin</span></h4></div><div id="mypanel" class="panel-body"></div></section></div>')
	        ...
	        delete: getDelete('/group_admins/'+id+'/@{{pidm}}')
				})
				...
				$('#content .btn-success').on('click', function() {
					mymodal = new modal({ 
						...
						content: '<div class="modal-search"></div><div class="list"></div>', title: 'Add Admin'}
					);

					mymodal.ref.on('click', '.filterable', function(e){
						$.post('/group_admins', {'group_id': group_id, 'pidm': e.currentTarget.dataset.pidm}, function(data) {
